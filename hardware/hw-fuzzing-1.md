...
所谓静态验证就是在不执行代码或者不进行硬件仿真的前提下进行正确性检验，比如 formal 就是其中的代表；而动态验证则是在执行代码的过程中进行正确性的检查。从这个角度上来看，前面提到的手动构造测试样例的方法也是动态验证的一种，但是受限于手动构造测试效率低下。为了可以自动化的产生测试样例，并进行自动化的动态验证而产生了 UVM 测试框架。
