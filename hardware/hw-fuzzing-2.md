...
覆盖率指标是对信息量的定量描述，我们可以用能描述模块特征的组件作为反映信息量的覆盖率指标。并且覆盖率指标的分布和信息量的分布越吻合，说明覆盖率指标对信息量的定量描述越准确，对模糊测试的引导效果越好。我们现在介绍一些最基本的软件、硬件覆盖率指标。

#### 软件覆盖率指标

软件覆盖率指标：可以用软件的控制流（代码执行的情况）、数据流（变量值的情况）情况作为覆盖率指标，可以用硬件监控或者软件插桩的方式收集覆盖率
* **指令覆盖率**：用被执行到的指令作为覆盖率指标；可以用硬件 monitor 监视指令执行的 trace，然后根据 trace 计算指令执行情况
* **代码块覆盖率**，**代码块转移覆盖率**：用被执行到的代码块、代码块之间的调转路径作为覆盖率，参见AFL；用软件插桩自动化记录
* **路径覆盖率**：用控制流执行的路径作为覆盖率，可以认为是代码块转移的进一步扩展
* **语句覆盖率**，**分支覆盖率**，**函数覆盖率**，**接口覆盖率**：用被执行的语句条数、分支数量、函数数量、接口数量作为覆盖率
* **判定覆盖率**：用每个分支布尔表达式的执行结果作为覆盖率，如 (branch1, true) 和 (branch1, false) 是两个不同的覆盖率
* **条件覆盖率**：用分支布尔表达式的条件数被至少使用一次的个数作为覆盖率，如 a || b 的 b 只有在 a==0 的时候被使用，因此使用与否是有差异性特征的
* 其他：也可以根据问题作具体设计，比如被访问的特殊变量、特殊变量的值等等

#### 硬件覆盖率指标

硬件覆盖率指标借鉴了软件覆盖率的指标。数据流的角度可以使用寄存器的值、线路的值、有限状态机的值等作为覆盖率；控制流的角度来说，可以根据寄存器的使能、mux 的选择子的值作为覆盖率。再考虑到 Verilog 等硬件语言可以用 if-else 等类软件写法描述和建模软件，并且在仿真的时候也使用类似的软件模型执行的，因此也可以存在行覆盖率、分支覆盖率、条件覆盖率等。

我们以行覆盖率为例。从硬件的视角看：当 en = 1 的时候，reg1、reg2 的使能信号被使能，选择子分别传递累加和递减电路的电平信号；en = 0 的时候，仅仅 reg1 的使能信号被使能，选择子传递递减电路电平信号。从软件的视角来看：当 en==1 的时候，执行第一个分支的第 3、4 两行；当 en==0 的时候，执行第二个分支的第 6 行。所以可以看到软件的分支执行等价于硬件的使能信号和 mux 选择信号的组合；行代码的执行等价于对应计算电路结果的传递和载入。因此可以用对应的软件行为来表征硬件行为。

```SystemVerilog
    always_ff@(posedge clk)begin    // 1 line
        if(en)begin                 // 2 line
            reg1 <= reg1 + 1'b1;    // 3 line
            reg2 <= reg2 - 1'b1;    // 4 line
        end else begin              // 5 line
            reg1 <= reg1 - 1'b1;    // 6 line
        end                         // 7 line
    end                             // 8 line
```

![对应的硬件电路流动](img/line-coverage.png)

* 行覆盖率：每个 Verilog 代码行描述的电路结果被传递、载入情况
* 分支覆盖率：一个 Verilog 分支描述的电路结果被传递、载入情况
* 条件覆盖：可以进入一个分支的所有情况的覆盖情况
* 翻转覆盖：每个线路的值情况，(wire, 0) 和 (wire, 1) 是独立的覆盖点
* 状态机覆盖：状态寄存器状态值的覆盖情况
* 断言覆盖：每个 property 被满足的情况

软件可以考虑用硬件监督或者软件插桩的方法收集覆盖率，对于硬件除了可以用模拟程序本身监控硬件执行情况之外；还可以用硬件插桩的方法，用定制化的硬件覆盖率。我们以 verilator 提供的行覆盖率为例，上面的例子经过 verilator 行覆盖率插桩之后，就如下所示。他会给每个行加入寄存器，当该行被执行时这个寄存器累加，从而统计每个行被执行的次数。EDA 工具可以用类似的方法提供各类覆盖率统计机制。
```SystemVerilog
    always_ff@(posedge clk)begin    // 1 line
        if(en)begin                 // 2 line
            reg1 <= reg1 + 1'b1;    // 3 line
            reg2 <= reg2 - 1'b1;    // 4 line
            cnt_line3 <= cnt_line3 + 1;
            cnt_line4 <= cnt_line4 + 1;
        end else begin              // 5 line
            reg1 <= reg1 - 1'b1;    // 6 line
            cnt_line6 <= cnt_line6 + 1;
        end                         // 7 line
    end                             // 8 line
```
