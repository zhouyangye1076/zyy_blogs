...
测试输入生成突变技术是处理器模糊测试领域的核心技术，决定了模糊测试状态搜索能力的上限。生成突变技术的质量高低取决于设计者对于测试输入内容认识的深浅，随着设计者对测试输入形态认识的深入，生成的测试输入的质量和效果可以得到快速地提高：
* RFUZZ 认为测试输入就是各个周期的**二进制序列组合**，因此采用了简单的二进制突变技术
* hw-fuzzing 考虑到了硬件数据交换需要**满足协议约束**，因此将被测模型的接口转化为 TL-UL 接口，然后用二进制突变技术随机构造 TL-UL 任务
* DifuzzRTL 意识到处理器测试的最小单元是指令序列和中断事件序列，因此根据指令集格式进行指令生成，**满足了指令内部的格式约束**，并且 DIfuzzRTL 意识到了程序的控制流约束问题，用模拟执行的方法进行指令排布，以尽量**满足控制流约束**要求
* TheHuzz 引入了突变优化算法，通过一个优化搜索算法**构造高覆盖率的测试指令突变**，提高覆盖率的增长速度
* Cascade 用**强制控制流约束**进行指令和指令参数生成，解决完整的控制流约束问题；此外，为了构造高质量的指令序列，Cascade 引入控制流和数据流纠缠技术，**提高控制流指令测试难度**
* Morfuzz 意识到**触发特定语义事件**需要构造特定指令间依赖，因此以事件为最小单位构造指令块；Morfuzz 用软硬件结合的控制流异常处理机制、页表处理机制彻底**解决控制流执行问题**，用软硬件结合的突变技术让测试程序可以不间断的进行突变测试，**满足突变的完成度、连续性、多样性要求**
* HyperFuzz 采用**形式化的方法**，针对特定的覆盖率进行定向的测试程序生成，为覆盖率引导测试程序生成提供行之有效的方案
* ChatFuzz/GenHuzz 采用**LLM 和强化学习的方法**进行测试程序生成，让 LLM 赋能处理器模糊测试

测试程序生成技术的优化过程是不断特化特殊输入组合生成的过程。我们希望可以覆盖处理器特殊功能，那么就要求生成的测试程序可以覆盖对应的功能点；对于二进制生成，这些特殊功能点的测试程序的生成概率太低了，因此需要专门构造生成算法来提高这些测试程序生成的概率。测试程序生成技术的发展就是在不断了解处理器的实现特性，不断发掘处理器内在的功能点，对功能点触发需要满足的输入约束进行建模，不断细化生成程序需要满足的约束条件的过程。从 RFUZZ->hw-fuzzing->DifuzzRTL->Cascade->Morfuzz，先后经历了满足二进制约束、数据传输协议约束、指令格式约束、中断事件约束、控制流约束、异常处理约束、页表设置约束、突变执行约束、语义功能块约束等一些列约束问题，从而不断细化处理器测试程序的特点，以期提高特殊事件触发的能力。对于约束的满足有三种解决思路：
* 构造复杂的测试程序生成算法，让生成的程序直接满足对应的约束，如 DifuzzRTL 的控制流摆放、Cascade 控制流和数据流纠缠、Morfuzz 的语义块构造等
* 在测试程序内部加入软件处理机制，让测试程序自己动态解决约束，如 DifuzzRTL 和 Morfuzz 的异常处理和页表设置机制、DifuzzRTL 的寄存器 dump 机制
* 在处理器内部加入硬件模块，用硬件功能逻辑解决约束，如 hw-fuzzing 的 TL-UL 接口、Morfuzz 的 morph 模块、magic mmio 模块等

相对于人为的将处理器功能点引入测试程序生成机制，另一个思路是让测试程序生成机制自己从处理器内部挖掘功能点。不过因为功能点过于抽象，这些工作用覆盖率指标做功能点的具象化。TheHuzz 通过覆盖率引导突变算法提高指令的覆盖率生成能力；HyperFuzz 用形式化方法根据处理器代码和覆盖点位置，自动求解测试程序生成；ChatFuzz 和 GenHuzz 用强化学习、NLP、LLM 的方法试图提高测试程序的覆盖率。想要更深入的挖掘处理器的功能点和语义信息，必须更深入的认识和分析处理器的设计实现，也就是必须让处理器设计实现作为测试程序生成机制的输入的一部分，并加入语义信息分析机制。

人为对于处理器程序约束的认识已经发展到了比较高的层次，想要进一步创新有很大的难度，未来的发展方向可以考虑让测试程序生成机制自动分析处理器，更深入的挖掘程序约束。因为形式化的方法可以计算处理器的逻辑实现，大模型对于硬件逻辑有很强的静态分析和语义处理能力，未来也许可以结合形式化技术和大模型技术，更自动化的分析处理器模型和功能，针对性的生成满足内部约束的测试程序。

### 处理器模糊测试的科研窘境

在中国乃至全世界，处理器模糊测试都是一个小众且冷门的领域。硬件领域在计算机科学与技术领域一直都是 hard 且 dirty 的，计算机专业只有很小一部分会选择系统、体系方向，系统、体系方向只有很少一部分会选择硬件方向，硬件方向只有很小一部分会选择硬件验证方向。和软件领域相比，硬件领域的工具链支持相对匮乏，开发、验证、调试的成本也要高至少一个数量级，并且还要考虑电气因素造成的未知影响，并且缺少专业的老师和同行的教育、技术支持。近年来，随着 AI 和 LLM 技术的蓬勃发展，更多的人才抛弃这些传统领域，试图去 AI/LLM 的汪洋大海里争取时代红利。虽然中国处理器行业在国家的扶持下、在各个机构和个人的努力下渐入佳境，但是在处理器验证领域沉淀有限，还是保留在传统的验证手段，对于处理器模糊测试的前沿技术需求有限，难以给出足够的商业需求和经费支持，产学研结合有限，是这个领域在市场经济主导的科研环境下难有起色。学习成本高、开发难度大、技术支持少、市场需求差，这是该科研领域面临的四大挑战。

和传统的硬件领域相比，处理器模糊测试领域反而是一个门槛低的新兴科研方向，这导致科研界的很多组都开始对这个领域灌水。许多工作通过指标包装的方式试图让自己显得很有吸引力，但是一但经过仔细推敲，就会让人对其数据的真实性和实际效果产生怀疑。有的工作通过挑选覆盖率指标让自己的覆盖率结果占优势，有些工作通过挖掘冷门的处理器来扩充自己的 BUG 数，甚至用已知的漏洞凑数。会议的 reviewer 往往缺少处理器模糊测试的科研经验，以至于很多工作可以蒙混过关，这也导致了处理器模糊测试领域作品质量良莠不齐，缺少有影响力、有现实意义的代表作支撑。我们需要更多处理器领域的有识之士重塑这个领域的科研氛围，促使处理器模糊测试领域更严谨、更热烈地发展壮大。

### 自述和告别

本人于 2023 年 12 月追随徐金焱师兄进入处理器模糊测试领域，学习和参与处理器瞬态执行漏洞模糊测试测试程序生成工作，于次年 6 月完成测试程序生成框架，并作为当年毕设圆满毕业。7-10 月期间，我们迭代实验细节、优化生成策略、敲定覆盖率指标，完成全部实验并提交论文。8 月，我在 RISCV 峰会香山分论坛汇报我们在香山处理器上的漏洞发现。12月，我参与 AE 仓库设计。2025 年 2-3 月完成论文 rebuttal。论文 Dejavuzz 最终被 ASPLOS 2025 收录，成为我的第一篇二作。之后，我决定继续深耕处理器模糊测试方向，将模糊测试技术和 XPU 热点结合，进行 XPU 的功能性模糊测试技术探索。3-4 月我对主流的开源 XPU 和相关硬件进行调研，4-5 月我系统学习整理主流处理器模糊测试的技术和理念，5 月我开始思考 XPU 模糊测试的特点和难点，并开始阅读 GEMMINI 原码寻找启发，是月底，在导师的强烈要求下放弃该领域，转而投入软硬件结合的安全机制领域。一晃又是 3 个月。一来，自己过去一年半积累了很多处理器模糊测试的技术和设计理念，任其慢慢淡忘甚是可惜；二来，作为给徐金焱、张幸智、洪奕迅等同行者的技术回馈，也为该领域的后进者留一份入门教材；最后，也是作为对过去一年半岁月的留恋，作为对处理器模糊测试领域的告别礼物：
* 迁移模糊测试技术，发源硬件模糊测试，作**RFUZZ**第一
* 集成硬件验证工具，设计总线中心接口，作**hw-fuzzing**第二
* 把握指令粒度突变，作控制寄存器指标，作**DifuzzRTL**第三
* 发现覆盖权重偏重，考虑覆盖时序变化，作**ProccesorFuzz**第四
* 引入突变优化算法，回归传统测试指标，作**TheHuzz**第五
* 强制解决控制约束，提出数据控制纠缠，作**Cascade**第六
* 语义粒度代码生成，软硬协同持续突变，作**Morfuzz**第七
* 形式求解测试程序，动态静态技术结合，作**HyperFuzz**第八
* 模型生成测试程序，强化提高覆盖指标，作**ChatFuzz**第九
* 指令切片编码学习，奖励考虑执行情况，作**GenHuzz**第十
* 植入漏洞构造平台，系统准衡过往工作，作**Encarsia**第十一
* 纲张目举继往开来，斑马萧萧挥手兹去，作**自述**第十二
